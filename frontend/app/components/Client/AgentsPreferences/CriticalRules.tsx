...
import { CriticalRuleFields } from 'Components/Issues/CriticalDialog';
...
/* "What's critical" — the centralized list Mehdi asked for on 07-28: one
   ...
   them to the LLM with per-user attribution and that is what makes "Critical to
   me" filterable ("we know this description came from Gabriel the designer and
   this one from Mehdi the engineer").

   Same shell as the journey-tag manager next door (Segmented + search + add
   over one table), so the two managers on this page are one component grammar.
   Editing and deleting follow the segments rule: your own rows get the pencil
   and trash, a teammate's row shows a disabled pencil whose tooltip names who
   can change it — the rule teaches instead of just refusing. */
...
function CriticalRules() {
  ...
  const openEdit = (row: CriticalRule) => {
    setEditing(row);
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  confirmDelete({
                    ...
                    consequence: orphans
                      ? t(
                          '{{count}} issue stops being critical for you.',
                          { count: orphans },
                        )
                      : t('No issue is currently critical because of it alone.'),
